...
class Target_ad_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->selected_columns = [
            ...
            "{$this->ads_table}.description",
            "{$this->ads_table}.hash_id",
            "{$this->ads_table}.ad_url",
            "{$this->ads_table}.action_text",
        ];
    }
    ...
    public function get($unit_obj)
    {
        ...
        $this->db->where("{$this->ads_table}.type", "ad");
        ...
        if ($result = $this->db->limit(1)->get()->row()) {
            $result->ad_domain = parse_url($result->ad_url, PHP_URL_HOST);
            $result->img_url = $result->filename ? image($result->filename)->url : "";
